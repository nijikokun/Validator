...
Validator.implement("type", function(options) {
    ...
    if (Array.isArray(options.value) && options.value.length > 0) {
        ...
        var dataType = Object.prototype.toString.call(options.data);
        var isType = false;
        var values = [];
        ...
        for (var i = 0; i < options.value.length; i++) {
            ...
            if (typeof options.value[i] === 'function') {
                value = options.value[i].name;
            }

            values.push(value);

            if (dataType.toLowerCase() == ("[object " + value + "]").toLowerCase())
                isType = true;
        }

        if (isType == false)
            options.error("Invalid parameter data type, expected: " + JSON.stringify(values));
